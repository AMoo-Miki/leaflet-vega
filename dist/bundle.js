/* @amoo-miki/leaflet-vega - v0.8.8 - Sun Sep 11 2022 20:20:03 GMT-0700 (Pacific Daylight Time)
...
(function (global, factory) {
  ...
  L__default["default"].vega = function vega(spec, options) {
    return new L__default["default"].VegaLayer(spec, options);
  };

  L__default["default"].VegaLayer = (L__default["default"].Layer ? L__default["default"].Layer : L__default["default"].Class).extend({
    options: {
    ...
    initialize: function initialize(spec, options) {
      ...
      L__default["default"].Util.setOptions(this, options); // expression parsing in Vega is global,
    ...
    _onAddAsync: function _onAddAsync(map) {
      ...
      return _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
        var _this2$options, vega, viewConfig, dataflow, oldLoad, onSignal;
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                _context.prev = 1;
                _this2._map = map;
                _this2._vegaContainer = L__default["default"].DomUtil.create('div', 'leaflet-vega-container');

                map._panes.overlayPane.appendChild(_this2._vegaContainer);

                _this2$options = _this2.options, vega = _this2$options.vega, viewConfig = _this2$options.viewConfig;
                  ...
                  switch (args.length) {
                    case 1:
                      {
                        ...
                        if (Array.isArray(arg) && arg.length === 2 && Array.isArray(arg[0]) && Array.isArray(arg[1])) {
                          ...
                          var _checkArray = checkArray(arg[0]),
                              _checkArray2 = _slicedToArray(_checkArray, 2),
                              lng1 = _checkArray2[0],
                              lat1 = _checkArray2[1];

                          var _checkArray3 = checkArray(arg[1]),
                              _checkArray4 = _slicedToArray(_checkArray3, 2),
                              lng2 = _checkArray4[0],
                              lat2 = _checkArray4[1];

                          map.fitBounds(L__default["default"].latLngBounds(L__default["default"].latLng(lat1, lng1), L__default["default"].latLng(lat2, lng2)));
                        } else {
                          // called with a center point and no zoom
                          var _checkArray5 = checkArray(arg);

                          var _checkArray6 = _slicedToArray(_checkArray5, 2);
                    ...
                    case 2:
                      if (Array.isArray(args[0])) {
                        ...
                        var _checkArray8 = _slicedToArray(_checkArray7, 2);
          ...
          while (1) {
            switch (_context2.prev = _context2.next) {
              ...
              case 2:
                ...
                L__default["default"].DomUtil.setPosition(_this3._vegaContainer, topLeft);
                size = map.getSize();
                center = map.getCenter();
                zoom = map.getZoom(); // Only send changed signals to Vega.
  ...
  L__default["default"].VegaLayer.version = version;
